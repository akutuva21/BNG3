...
class BNGBase(cement.Controller):
    ...
    def graphdiff(self):
        """
        Graph differencing subcommand.

        Calculates the differences between two graphml files generated by
        BioNetGen (e.g. contact maps) using a convenience function
        defined in core/main (which internally uses BNGGdiff).

        It will generate graphml files highlighting the differences and
        communalities based on the mode selected.
        """
        test_perl(app=self.app)
        graphDiff(self.app)
...
def main():
    with BioNetGen() as app:
        try:
            ...
            app.log.error("BNGError > %s" % e.args[0])
            app.exit_code = 1
            if app.debug is True:
                import traceback
